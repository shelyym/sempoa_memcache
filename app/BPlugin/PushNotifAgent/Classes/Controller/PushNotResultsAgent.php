...
class PushNotResultsAgent extends WebService{
                ...
                <td>
                    ...
                    <script>
                        $("#detail_<?=$cc->multicast_id;?>").click(function(){
                            ...
                            if(isHidden) {
                                $.get("<?=_SPPATH;?>PushNotResultsCaps/detail?id=<?=$cc->multicast_id;?>", function (data) {
...
//        pr($arrLogs);
        ...
        foreach($arrLogs as $log){
            ...
            ?>
            <div class="hasil">
                ID : <?=$acc->admin_id;?> <?=$acc->admin_nama_depan;?><br>
...
//        $arrAcc = $llacc->getOrderBy("macc_first_name ASC");
    ...
    <div class="container">
        ...
        <h1><?=$ps->camp_name;?></h1>
        Input Customer ID's <input class="form-control" type="text" id="ids" value="7,15">
    ...
    <script>
        $("#push").click(function(){
            ...
            $.post("<?=_SPPATH;?>PushNotResultsCaps/pusher?token=<?=$_GET['token'];?>",{
                camp_id : <?=$id;?>,
                ids : ids
            } ,function (data) {
//                alert(data);
...
//        pr($_POST);
        ...
        PusherCaps::sendUsingArrayAcc($ids,$ps, 1); //1 for testing
