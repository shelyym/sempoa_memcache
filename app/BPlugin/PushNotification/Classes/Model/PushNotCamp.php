...
class PushNotCamp extends Model{

    //

    //Nama Table
    public $table_name = "push__pushnot_program";

    //Primary
    public $main_id = 'camp_id';

//Default Coloms for read
    public $default_read_coloms = "camp_id,camp_client_id,camp_app_id,camp_start,camp_hour,camp_name,camp_title,camp_img,camp_msg,camp_url,camp_create_by,camp_active,camp_status,camp_send_date";

//allowed colom in CRUD filter
    public $coloumlist = "camp_id,camp_client_id,camp_app_id,camp_start,camp_hour,camp_name,camp_title,camp_img,camp_msg,camp_url,camp_create_by,camp_active,camp_status,camp_send_date,camp_acc_ids,camp_dev_ids";
    public $camp_id;
    ...
    public $crud_webservice_allowed = "camp_id,camp_client_id,camp_app_id,camp_start,camp_hour,camp_name,camp_title,camp_img,camp_msg,camp_url,camp_create_by,camp_active,camp_status,camp_send_date,camp_acc_ids,camp_dev_ids";
...
//
            ...
            $obj->Action = "<a class='btn  btn-default' href='"._SPPATH."PushNotResults/res?id=".$obj->camp_id."&token={$oauth}' target='_blank'>Results</a>
            <a class='btn  btn-default' href='"._SPPATH."PushNotResults/test?id=".$obj->camp_id."&token={$oauth}' target='_blank'>Test</a>";
        }
        return $return;
    }

    public function push_now($camp_id){

        //do push now...
        //ke pushnotcamp biasa..
    }

    public function test_push($acc_id,$app_name,$api_key){

        //acc_id ke bridge ?? dulu capsule
        //get device_id dari bridge dengan acc_id ini dan push...
        $dn = new DeviceModelCapsule();
        $arrDevices = $dn->getWhere("acc_id = '$acc_id'","device_id");
        $dev = array();
        foreach($arrDevices as $dv){
            $dev[] = $dv->device_id;
        }
        if(count($dev)>0)
            return self::pushAfteriOS($dev,$this->camp_name,$this->camp_id,$app_name,$api_key);
        else{
            return array("status"=>"no device found");
        }
    }

    static function pushAfteriOS($array_id,$msg,$camp_id,$app_name,$api_key)
    {
        if($api_key == "")die("no API key");
